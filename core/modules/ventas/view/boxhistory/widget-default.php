...
$operations = OperationData::getAllProductsBySellId($sell->id);
	foreach($operations as $operation){
		$product  = $operation->getProduct();
		$total2 += $operation->q*$product->price_in;
	}
}
		$total_total2 = $total-$total2;
		

?>
<h1>Total Neto: <?php echo "$ ".number_format($total_total,2,".",","); ?></h1>
<h1>Total Ganancia: <?php echo "$ ".number_format($total_total2,2,".",","); ?></h1>
	<?php
}else {
